...
public class UpdatePatchListInclude extends Task<Void> {
    private final VBox content;
    private final ArrayList<PatchDto> patches;
    private final String packageName;

    /**
     * Updates showed list to only display patches that have packageName in the package list or has an empty package list
     ...
     */
    public UpdatePatchListInclude(VBox content, ArrayList<PatchDto> patches, String packageName) {
    ...
    protected Void call() {
        ...
        patches.forEach(p -> {
            if (p.getPackageName().contains(packageName) || p.getPackageName().isEmpty())
